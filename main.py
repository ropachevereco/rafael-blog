...
def login():
    ...
    if form.validate_on_submit():
        ...
        if user is None:
            ...
            error = 'This password is incorrect, please try again.'
            flash(error)
            return redirect(url_for('login'))
...
def show_post(post_id):
    ...
    if form.validate_on_submit():
        if current_user.is_authenticated:
            ...
            return redirect(url_for('login'))
    return render_template("post.html", post=requested_post, current_user=current_user, form=form, comments=comments_data)
